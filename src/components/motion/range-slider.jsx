...
export function RangeSlider({
  ...
  const ticks =
    showTicks && steps > 0 && steps <= 50
      ? Array.from({ length: steps + 1 }, (_, i) => min + i * tickStep)
      : [];

  const commit = useCallback((next) => {
    const snapped = clamp(Math.round((next - min) / step) * step + min, min, max);
    if (snapped === lastCommitted.current) return;
    lastCommitted.current = snapped;
    if (!controlled) setInternal(snapped);
    if (haptic) navigator.vibrate?.(5);
    onValueChange?.(snapped);
  }, [controlled, haptic, onValueChange, min, max, step]);
