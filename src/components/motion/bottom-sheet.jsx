...
export function BottomSheet({
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      sheetRef.current?.querySelector("[autofocus], input, button, select, textarea, [tabindex]:not([tabindex='-1'])")?.focus();
    });
    return () => {
  ...
  const onKeyDown = (event) => {
    ...
    const focusable = [...sheetRef.current.querySelectorAll("input, button, select, textarea, [tabindex]:not([tabindex='-1'])")].filter((element) => !element.disabled);
